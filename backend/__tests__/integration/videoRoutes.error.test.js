...
describe('Video Routes Error Handling', () => {
  ...
  describe('POST /api/videos validation errors', () => {
    it('should handle missing video file', async () => {
      ...
      expect(response.body.message).toContain('Nessun file video caricato');
    });

    it('should handle invalid file type', async () => {
      const testFilePath = path.join(global.TEST_CONFIG.UPLOAD_DIR, 'test.txt');
      ...
      expect(response.status).toBe(400);
      expect(response.body.success).toBe(false);
      expect(response.body.message).toContain('Tipo di file non supportato');
    });
  ...
  describe('GET /api/videos/:id error cases', () => {
    ...
    it('should handle very large id', async () => {
      const response = await request(app)
        .get(`/api/videos/${Number.MAX_SAFE_INTEGER + 1}`);
  ...
  describe('DELETE /api/videos/:id error cases', () => {
    it('should handle deletion of already deleted video', async () => {
      ...
      const secondResponse = await request(app).delete(`/api/videos/${id}`);
