...
export const validateVideo = [
    ...
    body('description')
        ...
        .trim()
        .isLength({ max: 5000 })
        .withMessage('La descrizione non può superare 5000 caratteri'),

    body('publishAt')
        .optional()
        .isISO8601()
        .withMessage('La data di pubblicazione deve essere in formato ISO 8601')
        .custom(value => {
    ...
    body('hashtags')
        ...
        .custom((value, { req }) => {
            ...
            try {
                ...
                const validHashtags = hashtags.every(
                    tag =>
                        typeof tag === 'string' &&
                        tag.length >= 2 &&
                        tag.length <= 50 &&
                ...
                if (!validHashtags) {
                    throw new Error(
                        'Gli hashtag devono contenere solo lettere, numeri e underscore'
                    );
                }
    ...
    body('privacy')
        ...
        .withMessage('Privacy non valida'),

    body('language')
        .optional()
        .isLength({ min: 2, max: 5 })
        .withMessage('Codice lingua non valido')
        .matches(/^[a-z]{2}(-[A-Z]{2})?$/)
        .withMessage('Formato lingua non valido (es: it, en-US)'),
...
export const validateVideoId = [
    param('id').isInt().withMessage('ID video non valido'),
    handleValidationErrors,
];
