...
const fileFilter = (req, file, cb) => {
  ...
  if (allowedMimes.includes(file.mimetype)) {
    ...
    cb(new Error('Tipo di file non supportato. Carica solo file video (MP4, MOV, AVI, WMV)'), false);
  }
};
  ...
  if (err instanceof multer.MulterError) {
    if (err.code === 'LIMIT_FILE_SIZE') {
      return res.status(400).json({
        success: false,
        message: 'Il file supera la dimensione massima consentita (500MB)'
      });
    }
    return res.status(400).json({
      success: false,
  ...
  res.on('finish', () => {
    if (req.file && res.statusCode !== 201) {
      fs.unlink(req.file.path, (err) => {
        if (err) console.error('Errore durante la pulizia del file:', err);
      });
    }
  });
  next();
