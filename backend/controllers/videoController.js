...
class VideoController {
  ...
  async updateVideo(req, res) {
    try {
      ...
      if (req.body.metadata !== undefined) {
        try {
          ...
          return res.status(400).json({
            ...
            errors: [{
              field: 'metadata',
              message: 'Il campo metadata deve essere un JSON valido'
            }]
          });
        }
      }
      ...
      const result = await videoQueueService.updateVideo(videoId, updateData);
      if (result === 0) {
        return res.status(404).json({ 
          success: false, 
          message: 'Video non trovato' 
  ...
  async deleteVideo(req, res) {
    try {
      ...
      if (video.file_path) {
        await fs.unlink(video.file_path).catch(err => 
