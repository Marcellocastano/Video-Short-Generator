...
class VideoQueueService {
    ...
    async updateVideoStatus(id, status, youtubeId = null) {
        ...
        return await db('videos').where({ id }).update(updateData);
    }
    ...
    async updateVideo(id, videoData) {
        ...
        if (videoData.publishAt !== undefined)
            updateData.publish_at = videoData.publishAt;
        if (videoData.metadata !== undefined)
            updateData.metadata = JSON.stringify(videoData.metadata);

        return await db('videos').where({ id }).update(updateData);
    }

    async getAllVideos() {
        return await db('videos').select('*').orderBy('created_at', 'desc');
    }

    async deleteVideo(id) {
        return await db('videos').where({ id }).del();
    }

    async getAllVideosOrderedByPublishDate() {
        return await db('videos').orderBy('publish_at', 'desc');
    }
}
